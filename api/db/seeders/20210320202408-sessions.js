...
const KEYWORDS_NEGATIVE = [
  'soutenance',
  'Réunion MBDS',
  'Semaine de cours MBDS',
  'RENDU RAPPORT',
  'Pause déjeuner',
  'PAUSE DEJEUNER',
  'Cours MBDS à programmer',
]
const GOOGLE_CALENDAR_URL =
  'https://www.googleapis.com/calendar/v3/calendars/qrohj3qr45o45a4enu1i686kro@group.calendar.google.com/events?key=AIzaSyBU2cBOzrFlAzAe_vhaS4iIF5MOXj-zvZY&timeMin=' +
  GOOGLE_CALENDAR_PLANNING_TIME_MIN
...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    let sessions = scheduled_sessions.reduce((cumul, curr, i) => {
      if (!curr.summary) return cumul
      const formattedSummary = sanitize(curr.summary)
      let condition =
        !!curr.start &&
        !!curr.end &&
        (KEYWORDS_NEGATIVE.filter(
          (kw) => formattedSummary.indexOf(sanitize(kw)) >= 0
        ).length <= 0 ||
          KEYWORDS_POSITIVE.filter(
            (kw) => formattedSummary.indexOf(sanitize(kw)) >= 0
          ).length > 0)
      if (condition)
        cumul.push({
